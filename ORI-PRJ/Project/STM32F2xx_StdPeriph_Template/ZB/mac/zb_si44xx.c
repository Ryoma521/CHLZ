...
*                                                                          *
****************************************************************************
PURPOSE: siliconlab si4463 specific
*/
#ifdef ZB_SI44XX
#include "zb_common.h"
#include "zb_si44xx.h"
...
/* ds-2400 4.3.1. Transmit Packet in Normal FIFO */
  ...
  zb_si4463_fifo_write(fifo_addr, buf); // Ryoma
  ...
  if (need_tx)
  {
    ...
    */
    ...
    si446x_start_tx(0u, 0x30,  sizeof(buf));
    if (need_ack)
    {
...
void zb_si4463_fifo_write(zb_uint16_t long_addr, zb_buf_t *buf) ZB_SDCC_REENTRANT
{
