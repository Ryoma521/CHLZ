...
void SI4463_NVIC_Configuration(void)
{
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 2;                   //NIRQ初始化
    ...
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 4;           //无关
...
void SI4463_Enable_NIRQ_Int(void)
{
    ...
    NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);               // 抢占式优先级别
    NVIC_InitStructure.NVIC_IRQChannel            = NIRQ_IRQChannel; //指定中断源
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 2;           //NIRQ使能
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2;          // 指定响应优先级别1
    NVIC_InitStructure.NVIC_IRQChannelCmd         = ENABLE;
    NVIC_Init(&NVIC_InitStructure);
}
...
void SI4463_Disable_NIRQ_Int(void)
{
    ...
    NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);               // 抢占式优先级别
    NVIC_InitStructure.NVIC_IRQChannel            = NIRQ_IRQChannel; //指定中断源
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 2;                   //NIRQ禁用
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2;           // 指定响应优先级别1
...
void EXTILineNIRQ_Config(void)
{
  ...
  NVIC_InitStructure.NVIC_IRQChannel            = NIRQ_IRQChannel; //指定中断源
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;        //NIRQ设置
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 2;               // 指定响应优先级别1
  NVIC_InitStructure.NVIC_IRQChannelCmd         = ENABLE;
  NVIC_Init(&NVIC_InitStructure);
...
void TriggerLine_Config(void)
{
  ...
  /* Connect EXTI Line0 to PB1 pin */
  SYSCFG_EXTILineConfig(EXTI_PortSourceGPIOA, EXTI_PinSource2);       //选择 GPIO管脚用作外部中断线路
  

  /* Configure EXTI Line0 */
  EXTI_InitStructure.EXTI_Line    = EXTI_Line2;                 //外部中断线
  EXTI_InitStructure.EXTI_Mode    = EXTI_Mode_Interrupt;        //中断模式
  EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Rising;       //中断触发方式
  EXTI_InitStructure.EXTI_LineCmd = ENABLE;                     //打开中断
  EXTI_Init(&EXTI_InitStructure);
  ...
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;        //NIRQ设置
